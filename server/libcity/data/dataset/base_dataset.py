...
class BaseDataset(AbstractDataset):
    ...
    def get_data(self, testonly=False):
...
class TrajectoryProcessingDataset(Dataset):
    ...
    def __init__(self, data_name, data_type, vocab, seq_len=512,
        ...
        if data_type == 'test':
            self.data_path = 'raw_data/{}/{}_{}2.csv'.format(data_name, data_name, data_type)
            self.cache_path = 'NOT_EXIST'
            self.temporal_mat_path = 'NOT_EXIST'
        else:
        ...
        self._load_data(data_type == 'test')

    def _load_data(self, reload=False):
        # 此处是加载数据的代码
        if not reload and os.path.exists(self.cache_path) and os.path.exists(self.temporal_mat_path) :
        # if False:
            print(self.temporal_mat_path)
    ...
    # 非常重要！讲述了如何从原始数据变为可以Dataset可以读取的数据
    def data_processing(self, origin_data, desc=None, cache_path=None, tmat_path=None):
        ...
        for i in tqdm(range(sub_data.shape[0]), desc=desc):
            ...
            usr_id = traj['usr_id']
            # 下面这一行进行了一个神奇的转换，转换规则是通过读取`vocab_porto_True_1_merge.pkl`得到的
    ...
    def __getitem__(self, ind):
        ...
        # print(traj_ind)
        # print(temporal_mat)
        ret = torch.LongTensor(traj_ind), torch.LongTensor(temporal_mat)
        return ret
